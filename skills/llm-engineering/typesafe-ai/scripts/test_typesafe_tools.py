...
def run_suite():
    ...
    router_cases = [
        ("We need to audit our API endpoints for JWT auth bypass and SQL injection", "security-red-teamer", "security-vulnerability-scanner"),
        ("Rewrite this AI-generated blog post so it has voice and burstiness", "technical-writer-scribe", "human-writer"),
        ("Plan our M001 milestone into vertical slices with UAT verification", "lead-orchestrator", "get-shit-done"),
        ("Diagnose why the Redis worker has a race condition on key expiration", "fullstack-engineer", "bug-hunter"),
        ("Hi there, how are you doing today?", "general", "none"),
    ]

    for prompt, exp_persona, exp_skill in router_cases:
    ...
    doc = (
        ...
        "It supports columnar vector execution and integrates natively with Python and Apache Arrow."
    )

    verifier_cases = [
        ("DuckDB is an embedded columnar database with native Python support.", "PASS"),
        ("DuckDB is a distributed key-value store requiring 5 server nodes.", "REJECT_AND_RETRY"),
        ("DuckDB does not support Python or Arrow integration.", "REJECT_AND_RETRY"),
    ]

    for claim, exp_action in verifier_cases:
        report = verifier.verify(claim, doc)
        assert report.recommended_action == exp_action, f"Verifier failed: expected {exp_action}, got {report.recommended_action}"
